    ...
    <div class="container-fluid bg-dark">
        <div class="row" style="padding-bottom:10px">
            <div class=" col-md-1.5 col-xs-4" style="padding: 10px;">
                <button type="button" class="btn  btn-rounded btn-outline-danger active">Trending Songs</button>
            </div>
            <div class=" col-md-1.5 col-xs-4" style="padding: 10px;">
        ...
        try{
            ...
            PreparedStatement psmt=con.prepareStatement("select * from (select * from songview order by views desc) where rownum <= 30");
            ResultSet rs=psmt.executeQuery();
            while(rs.next())
            {
            	int songid=rs.getInt("songid");
            	
            	PreparedStatement psmt2=con.prepareStatement("select * from song where songid=?");
            	psmt2.setInt(1,songid);
            	ResultSet rs2=psmt2.executeQuery();
            	rs2.next();
            	String songname=rs2.getString("songname");
            	int albumid=rs2.getInt("albumid");
            	PreparedStatement psmt3=con.prepareStatement("select * from album where albumid=?");
            	psmt3.setInt(1,albumid);
            	ResultSet rs3=psmt3.executeQuery();
            	rs3.next();
            	String iaddress=rs3.getString("iaddress");
            	String link="songpage.jsp?songid="+songid+"";
            	%>
            	
                <div class="col-md-3">
                  <img src="download.jsp?albumid=<%=albumid%>" onclick="location.href='<%=link %>'" id="icon" style="cursor:pointer" alt="">
                  <h5  class="text-warning"><%=songname %></h5>
                </div>
                <% 
